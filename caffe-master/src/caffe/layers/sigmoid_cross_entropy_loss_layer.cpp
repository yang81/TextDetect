...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_cpu(
  ...
  int valid_count = 0;
  Dtype loss = 0;
	const Dtype *balance_data = bottom[2]->cpu_data();
	Dtype balance = balance_data[0];

	const Dtype *border_weight = bottom[3]->cpu_data();
  ...
  for (int i = 0; i < bottom[0]->count(); ++i) {
    ...
    if (has_ignore_label_ && target_value == ignore_label_) {
      continue;
		...
		Dtype balance_factor = 0;

		Dtype weight = border_weight[i];

		if (target_value == 0){
			balance_factor = balance + (1 - balance) * weight;
		}else{
			balance_factor = 1 - balance;
		}
		
    loss -= balance_factor * (input_data[i] * (target[i] - (input_data[i] >= 0)) -
