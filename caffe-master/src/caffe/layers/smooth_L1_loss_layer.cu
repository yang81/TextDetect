...
void SmoothL1LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (has_inside_weights_) {
    // apply "inside" weights
    caffe_gpu_mul(
        count,
        bottom[has_inside_weights_]->gpu_data(),
        diff_.gpu_data(),
  ...
  if (has_outside_weights_) {
    // apply "outside" weights
    caffe_gpu_mul(
        count,
        bottom[has_outside_weights_]->gpu_data(),
        errors_.gpu_data(),
  ...
  caffe_gpu_dot(count, ones_.gpu_data(), errors_.gpu_data(), &loss);
	caffe_gpu_dot(count, ones_.gpu_data(), bottom[has_outside_weights_]->gpu_data(), &n);

	
	int int_normalizer = static_cast<int>(n);
	normalizer_ = (int_normalizer == 0)?1:n;
	//normalizer_ = bottom[0]->width() * bottom[0]->height();
	if (use_norm_){
  ...
  for (int i = 0; i < 2; ++i) {
    if (propagate_down[i]) {
      ...
      Dtype alpha = 0;
			...
			if (use_norm_){
				alpha = sign * top[0]->cpu_diff()[0] / normalizer_;
			}else{
				alpha = sign * top[0]->cpu_diff()[0];
			}
      ...
      if (has_inside_weights_) {
        ...
        caffe_gpu_mul(
            ...
            bottom[i]->mutable_gpu_diff());
				...
				/*
				Dtype sum_d = 0;
				caffe_gpu_asum(count, bottom[i]->gpu_diff(), &sum_d);
				printf("diff sum : %f", sum_d/normalizer_);*/
      }
